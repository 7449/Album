...
import com.album.core.AlbumCamera.OPEN_CAMERA_PERMISSION_ERROR
import com.album.core.AlbumCamera.OPEN_CAMERA_SUCCESS
...
private fun openCamera(any: Any, cameraUri: Uri, video: Boolean): Int {
    ...
    cameraActivity?.let {
        ...
        if (any is Activity) {
            any.startActivityForResult(intent, AlbumCamera.OPEN_CAMERA_REQUEST_CODE)
        }
        if (any is Fragment) {
            any.startActivityForResult(intent, AlbumCamera.OPEN_CAMERA_REQUEST_CODE)
        }
...
fun AlbumBaseActivity.finishCamera(path: String) {
    val bundle = Bundle()
    bundle.putString(AlbumCamera.CUSTOMIZE_CAMERA_RESULT_PATH_KEY, path)
    val intent = Intent()
