...
class WeChatPrevSelectAdapter(
    ...
    fun updateSelect(entities: List<ScanMinimumEntity>) {
        list.clear()
        entities.forEach { list.add(it.copy(isSelected = false)) }
        notifyDataSetChanged()
    }

    fun findPosition(scanEntity: ScanMinimumEntity): Int {
        return list.indexOfFirst { it.id == scanEntity.id }
    }

    fun addSelect(entity: ScanMinimumEntity) {
    ...
    fun refreshItem(scanEntity: ScanMinimumEntity) {
