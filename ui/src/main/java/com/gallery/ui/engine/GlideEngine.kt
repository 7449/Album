...
internal fun FrameLayout.displayGallery(width: Int, height: Int, galleryEntity: ScanEntity) {
    ...
    Glide.with(context).load(galleryEntity.delegate.externalUriExpand).apply(RequestOptions().placeholder(R.drawable.ic_gallery_default_loading).error(R.drawable.ic_gallery_default_loading).centerCrop().override(width, height)).into(imageView)
...
internal fun FrameLayout.displayGalleryPrev(scanEntity: ScanEntity) {
    ...
    Glide.with(context).load(scanEntity.delegate.externalUriExpand).into(imageView)
    addView(imageView)
}