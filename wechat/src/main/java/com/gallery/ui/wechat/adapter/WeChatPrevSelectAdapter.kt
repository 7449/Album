...
class WeChatPrevSelectAdapter(
    ...
    fun updateSelect(entities: List<ScanFileEntity>) {
        list.clear()
        entities.forEach { list.add(it.copy(isSelected = false)) }
        notifyDataSetChanged()
    }

    fun findPosition(scanEntity: ScanFileEntity): Int {
        return list.indexOfFirst { it.id == scanEntity.id }
    }

    fun addSelect(entity: ScanFileEntity) {
    ...
    fun refreshItem(scanEntity: ScanFileEntity) {
